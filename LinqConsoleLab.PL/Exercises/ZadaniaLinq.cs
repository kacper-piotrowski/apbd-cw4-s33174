...
public sealed class ZadaniaLinq
{
    ...
    public IEnumerable<string> Zadanie01_StudenciZWarszawy()
    {
        ...
        var query = from s in DaneUczelni.Studenci
            ...
            //select new { s.NumerIndeksu, s.Imie, s.Nazwisko, s.Miasto }; //tryb zwracania zmiennej anonimowej
    ...
    {
        var method2 = DaneUczelni.Studenci.Select(s=>s.Email);
        return method2;
    ...
    {
        var method3 = DaneUczelni.Studenci
    ...
    {
        var method4 = DaneUczelni.Przedmioty
            .Where(p => p.Kategoria == "Analytics")
            .Select(p => $"{p.Nazwa}, {p.DataStartu}")
            .FirstOrDefault();
        return method4!=null?[method4]:["Nie ma takikego przedmiotu!"];
    ...
    {
        var method5 = DaneUczelni.Zapisy
            .Where(z => z.CzyAktywny == false)
            .Select(z => z)
            .Any();
        return method5 ? ["True"] : ["False"];
    ...
    {
        var method7 = DaneUczelni.Zapisy.Count(z => z.CzyAktywny);
        return [method7.ToString()];
    ...
    {
        var method8 = DaneUczelni.Studenci.OrderBy(s=>s.Miasto).Select(s=>s.Miasto).Distinct();
        return method8;
    ...
    {
        var method9 = DaneUczelni.Zapisy
    ...
    {
        var method10 = DaneUczelni.Przedmioty
            .OrderBy(p => p.Nazwa)
    ...
    {
        var query12 = from z in DaneUczelni.Zapisy
    ...
    {
        var method14 = DaneUczelni.Zapisy
            .Join(DaneUczelni.Przedmioty, z=> z.PrzedmiotId, p => p.Id, (z, p) => new { z, p })
            .Where(pz => pz.z.OcenaKoncowa!= null)
            .GroupBy(pz => pz.p.Nazwa)
            .Select(pz => $"{pz.Key}, {pz.Average(z=> z.z.OcenaKoncowa)}");
        return method14;
    ...
    {
        var method15 = DaneUczelni.Prowadzacy
    ...
    {
        var method16 = DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy, s=>s.Id, z=>z.StudentId, (s, z) => new { s, z })
            .Where(z => z.z.OcenaKoncowa!= null)
            .GroupBy(sz=> new {sz.s.Imie, sz.s.Nazwisko})
            .Select(sz => $"{sz.Key.Imie}, {sz.Key.Nazwisko}, {sz.Max(z=>z.z.OcenaKoncowa)}");
        return method16;
    ...
    {
        var methodwyz1 = DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy, s=>s.Id, z=>z.StudentId, (s, z) => new { s, z })
    ...
    {
        var methodwyz3 = DaneUczelni.Prowadzacy
            .Join(DaneUczelni.Przedmioty, pr => pr.Id, p => p.ProwadzacyId, (pr, p) => new { pr, p })
            .Join(DaneUczelni.Zapisy, prp => prp.p.Id, z => z.PrzedmiotId, (prp, z) => new { prp, z })
            .Where(prpz => prpz.z.OcenaKoncowa != null)
            .GroupBy(prpz => new {prpz.prp.pr.Imie, prpz.prp.pr.Nazwisko})
            .Select(prpz =>
                $"{prpz.Key.Imie}, {prpz.Key.Nazwisko}, {prpz.Average(prpz2=>prpz2.z.OcenaKoncowa)}");
        return methodwyz3;
    ...
    {
        var  methodwyz4 = DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy, s=>s.Id, z=>z.StudentId, (s, z) => new { s, z })
            .Where(sz=>sz.z.CzyAktywny)
